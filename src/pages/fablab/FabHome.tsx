...
import { LayoutDashboard, Package, Calendar, Lightbulb, Wrench, BarChart3, Users, FolderOpen, AlertTriangle } from 'lucide-react';
...
export function FabHome() {
  ...
  if (user?.role === 'admin') {
    features = [
      ...
      { icon: <AlertTriangle size={20} />, title: 'Manutenção', desc: 'Tickets de máquinas e equipamentos', path: '/fablab/maintenance' },
      { icon: <FolderOpen size={20} />, title: 'Arquivos', desc: 'STL, G-Code, SVG e DXF', path: '/fablab/files' },
    ...
    features = [
      { icon: <AlertTriangle size={20} />, title: 'Manutenção', desc: 'Reportar problema de máquina', path: '/fablab/maintenance' },
      { icon: <FolderOpen size={20} />, title: 'Arquivos', desc: 'STL, G-Code, SVG e DXF', path: '/fablab/files' },
    ...
    features = [
      { icon: <FolderOpen size={20} />, title: 'Arquivos', desc: 'Baixe STL, G-Code e SVG', path: '/fablab/files' },
  ...
  return (
    <PageTransition>
      ...
      >
        ...
        <h1 className="text-3xl md:text-4xl font-serif mb-2">FabLab</h1>
        <p className="text-sm opacity-85 max-w-md leading-relaxed">
          Bem-vindo, {user?.name?.split(' ').slice(0, 2).join(' ')}. Sistema de gerenciamento do laboratório de fabricação digital.
        </p>
      </div>
